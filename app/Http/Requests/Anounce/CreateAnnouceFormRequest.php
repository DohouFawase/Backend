...
class CreateAnnouceFormRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            // 'property_type_id' => ['required', 'uuid', 'exists:property_types,id'], // Si tu as cette table
            ...
            'description' => ['required', 'string', 'min:50'], // Description plus longue requise
            ...
            'currency' => ['required', 'string', 'max:10'], // Ex: XOF, USD
            
            // Commission et dépôt seulement pour la location
            ...
            // Périodicité seulement pour la location
            'periodicity' => [
...
'             property_type_id' => ['required', 'uuid', 'exists:property_types,id'],
            ...
            // 🎯 NOUVEAU : equipments est un tableau d'UUIDs, il doit être présent et chaque ID doit exister
            'equipments' => ['required', 'array', 'min:1'],
            'equipments.*' => ['uuid', 'exists:equipments,id'],
            // --- Étape 5 : Médias
            // Les photos doivent être traitées comme des fichiers uploadés (s'il y a un champ files)
            // Si on reçoit juste un JSON de noms de fichiers, on vérifie la structure
     ...
     * C'est utile pour nettoyer ou définir des valeurs par défaut.
     */
    protected function prepareForValidation(): void
    {
        // Convertir la chaîne 'true'/'false' des booléens en booléen réel si nécessaire
        $this->merge([
